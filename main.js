...
function refreshData(){
    ...
    EGGS_TO_HATCH_1TURTLE(function(eggs){
        eggstohatch1=eggs
    });
    getMyEggs(function(eggs){
        ...
        secondsuntilfull=eggstohatch1-eggs/lastNumTurtle
        console.log('secondsuntilfull ',secondsuntilfull,eggstohatch1,eggs,lastNumTurtle)
    ...
    getMyTurtle(function(turtle){
        lastNumTurtle=turtle
        var gfsdoc=document.getElementById('getfreeturtle')
        if(turtle>0){
        ...
        var allnumturtle=document.getElementsByClassName('numturtle')
        for(var i=0;i<allnumturtle.length;i++){
            if(allnumturtle[i]){
                allnumturtle[i].textContent=translateQuantity(turtle,0)
            }
        }
        var productiondoc=document.getElementById('production')
        productiondoc.textContent=formatEggs(lastNumTurtle*60*60)
    });
    updateBuyPrice()
    updateSellPrice()
	updateTurtlemasterPrice()
	updateCurrentTurtlemaster()
...
function updateEggNumber(eggs){
    var hatchturtlequantitydoc=document.getElementById('hatchturtlequantity')
    hatchturtlequantitydoc.textContent=translateQuantity(eggs,0)
...
function liveUpdateEggs(){
    if(lastSecondsUntilFull>1 && lastNumEggs>=0 && lastNumTurtle>0 && eggstohatch1>0){
        currentTime=new Date().getTime()
        if(currentTime/1000-lastHatchTime>eggstohatch1){
            return;
        }
        difference=(currentTime-lastUpdate)/1000
        additionalEggs=Math.floor(difference*lastNumTurtle)
...
function updateTurtlemasterPrice(){
    var turtlemasterpricedoc=document.getElementById('turtlemasterprice')
	getTurtlemasterReq(function(req) {
		turtlemasterpricedoc.textContent = translateQuantity(req, 0);
	});
}


function updateCurrentTurtlemaster(){
    var currentturtlemasterdoc=document.getElementById('currentturtlemaster')
    //eggstobuydoc.textContent='?'
	currentturtlemaster.textContent=ceoAddress()
}

function getFreeTurtle2(){
    var ethtospenddoc=0.001//document.getElementById('freeturtlespend')
    weitospend=web3.toWei(ethtospenddoc,'ether')
    getFreeTurtle(weitospend,function(){
...
function disableButtons(){
    var allnumturtle=document.getElementsByClassName('btn-lg')
    for(var i=0;i<allnumturtle.length;i++){
        if(allnumturtle[i]){
            allnumturtle[i].style.display="none"
        }
    }
}
function enableButtons(){
    ...
    for(var i=0;i<allnumturtle.length;i++){
        if(allnumturtle[i]){
            allnumturtle[i].style.display="inline-block"
        }
    }
}
